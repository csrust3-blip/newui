...
function LegendUI:CreateWindow(config)
    ...
    local BASE_WIDTH = 717
    local BASE_HEIGHT = 646

    local Root = Instance.new("Frame")
    Root.Name = "Root"
    Root.Size = UDim2.fromOffset(BASE_WIDTH, BASE_HEIGHT)
    Root.Position = UDim2.new(0.5, -BASE_WIDTH / 2, 0.5, -BASE_HEIGHT / 2)
    Root.BackgroundColor3 = Theme.Background
    Root.BorderSizePixel = 0
    Root.Parent = ScreenGui
    Root.ClipsDescendants = true

    -- Auto-scale the whole window to fit the viewport (handles mobile/small screens)
    ...
    local function updateScale()
        ...
        local scaleX = (viewport.X * 0.94) / BASE_WIDTH
        local scaleY = (viewport.Y * 0.94) / BASE_HEIGHT
        local scale = math.min(scaleX, scaleY, 1)

        local isMobile = UserInputService.TouchEnabled and not UserInputService.MouseEnabled

        if isMobile then
            -- On phones/tablets the window would otherwise render tiny relative to
            -- finger precision, so bias the minimum scale upward a bit.
        ...
        UserInputService.InputChanged:Connect(function(input)
            if dragging
                and (input.UserInputType == Enum.UserInputType.MouseMovement
                    or input.UserInputType == Enum.UserInputType.Touch) then
                local delta = (input.Position - dragStart) * RootScale.Scale
    ...
    -- Floating toggle button: shows/hides the whole window, and survives it being hidden
    local ToggleButton = Instance.new("TextButton")
    ToggleButton.Name = "ToggleButton"
    ToggleButton.Size = UDim2.fromOffset(44, 44)
    ...
    ToggleButton.MouseEnter:Connect(function()
        tween(ToggleButton, { BackgroundColor3 = Theme.ToggleOff }, 0.1)
    end)

    ToggleButton.MouseLeave:Connect(function()
        tween(ToggleButton, { BackgroundColor3 = Theme.CardBg }, 0.1)
    end)

    ToggleButton.MouseButton1Click:Connect(function()
        Root.Visible = not Root.Visible
        ToggleButton.BackgroundColor3 = Root.Visible and Theme.ToggleOff or Theme.CardBg
    end)
...
function LegendUI._buildBox(parent, title)
    ...
    -- Plain info/section text. Height grows automatically to fit wrapped text,
    -- so it's safe to use for short labels or longer paragraphs alike.
    function Box:AddLabel(text, textColor)
        local Row = newRow(20)
